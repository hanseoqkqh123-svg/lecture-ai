        ...
        <div className="app">
            <div className="container" style={{ paddingBottom: '100px' }}>
                        ...
                        <div className="list">
                            {history.length === 0 ? <p className="emptyText">아직 기록된 강의가 없습니다. 새로운 녹음을 시작해보세요!</p> : (
                                history.map((item, idx) => (
                                    <div key={item.id || idx} className="itemBox" style={{ cursor: 'pointer', transition: 'background-color 0.2s' }}
                                        ...
                                        <div className="dateText">{item.createdAtText || item.createdAt}</div>
                ...
                {currentScreen === "recording" && (
                    <>
                        <div style={{ marginBottom: '15px' }}><button onClick={() => { setCurrentScreen("home"); setSelectedHistory(null); setAnswers({}); }} className="secondaryBtn">← 홈으로 돌아가기</button></div>
                        <div className="grid">
                            <div>
                                <div className="card">
                                    <h2>1. 강의 내용 입력</h2>
                ...
                {currentScreen === "detail" && selectedHistory && (
                    <>
                        <div style={{ marginBottom: '15px' }}><button onClick={() => { setCurrentScreen("home"); setSelectedHistory(null); setAnswers({}); }} className="secondaryBtn">← 목록으로 돌아가기</button></div>
                        <div className="card" style={{ marginBottom: '20px', backgroundColor: '#f8f9fa', border: '1px solid #e9ecef' }}><h2 style={{ margin: '0 0 10px 0', color: '#212529' }}>{selectedHistory.title}</h2><p className="subText" style={{ margin: 0 }}>생성 일시: {selectedHistory.createdAtText || selectedHistory.createdAt}</p></div>
                        <div className="grid">
                            <div>
                            ...
                            <div>
                                ...
                                <div className="card"><h2>다시 풀어보는 복습 퀴즈</h2>
                                    ...
                                    {(selectedHistory.savedQuiz || []).length > 0 && <div className="scoreBox"><div className="scoreTitle">현재 점수</div><div>{savedResult.correct} / {savedResult.total} 정답 · {savedResult.score}점</div></div>}
                                </div>
                            </div>
                        </div>
                    </>
